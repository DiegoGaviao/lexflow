...
DO $$ BEGIN 
  IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'lf_app_role') THEN
    CREATE TYPE public.lf_app_role AS ENUM ('admin', 'advogado', 'operacional');
  END IF;
END $$;
...
CREATE OR REPLACE FUNCTION public.lf_has_role(_user_id UUID, _role lf_app_role)
RETURNS BOOLEAN
...
CREATE POLICY "Admins can view all profiles" ON public.lf_profiles FOR SELECT USING (public.lf_has_role(auth.uid(), 'admin'::lf_app_role));

-- lf_user_roles policies
CREATE POLICY "Users can view own roles" ON public.lf_user_roles FOR SELECT USING (auth.uid() = user_id);
CREATE POLICY "Admins can manage roles" ON public.lf_user_roles FOR ALL USING (public.lf_has_role(auth.uid(), 'admin'::lf_app_role));

-- lf_processos policies
...
CREATE POLICY "Admins can view all processos" ON public.lf_processos FOR ALL USING (public.lf_has_role(auth.uid(), 'admin'::lf_app_role));

-- lf_movimentacoes policies
...
BEGIN
  ...
  INSERT INTO public.lf_user_roles (user_id, role) VALUES (NEW.id, 'advogado'::lf_app_role);
